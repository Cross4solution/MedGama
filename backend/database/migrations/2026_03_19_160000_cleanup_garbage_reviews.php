...
return new class extends Migration
{
     ...
     * MySQL/TiDB: uses REGEXP operator
     * SQLite: uses IN/LIKE patterns
     */
    public function up(): void
    {
        ...
        DB::table('doctor_reviews')
            ->where(function ($q) use ($driver) {
                ...
                if ($driver === 'pgsql') {
                    $q->orWhereRaw("LOWER(comment) ~ '^[a-z]{1,6}$'")
                      ->orWhereRaw("comment ~ '^[A-Z]{3,}$'")
                    ...
                    // MySQL / TiDB
                    $q->orWhereRaw("LOWER(comment) REGEXP '^[a-z]{1,6}$'")
                      ->orWhereRaw("comment REGEXP '^[A-Z]{3,}$'")
