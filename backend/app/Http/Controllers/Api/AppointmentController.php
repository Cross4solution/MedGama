...
class AppointmentController extends Controller
{
    ...
    private function isAppointmentParty(Appointment $appointment, $user): bool
    {
        return in_array($user->id, array_filter([
            $appointment->patient_id, $appointment->doctor_id, $appointment->clinic_id,
        ]), true) || in_array($user->role_id, ['superAdmin', 'saasAdmin'], true);
    }
    ...
    public function downloadSharedDocument(Appointment $appointment, string $documentId, Request $request): mixed
    {
        ...
        HealthDataAuditLog::log(
            accessorId: $user->id,
            patientId: $appointment->patient_id,
            resourceType: 'medical_document_downloaded',
            resourceId: $doc->id,
        );
