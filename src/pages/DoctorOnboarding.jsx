...
import {
  ...
  Plus, X, Loader2, Upload, Camera, PartyPopper, Eye
} from 'lucide-react';
...
export default function OnboardingWizard() {
  ...
  const [languages, setLanguages] = useState(['Turkish', 'English']);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (p) {
          if (p.onboarding_completed) { navigate('/crm'); return; }
      ...
      if (step === 0) {
        ...
        if (avatarFile) {
          try {
            ...
            const newAvatar = avatarRes?.data?.avatar || avatarRes?.avatar;
      ...
      <div className="flex-1 max-w-3xl w-full mx-auto px-4 sm:px-6 py-8">
        ...
        {step === 0 && (
          <div className="space-y-5">
            ...
            {/* Languages */}
            <div>
              <label className={labelCls}>{t('onboarding.languages')}</label>
              <div className="flex flex-wrap gap-2 mb-2">
                {languages.map((l, i) => (
                  <span key={i} className="inline-flex items-center gap-1 px-3 py-1 bg-teal-50 text-teal-700 rounded-full text-xs font-medium border border-teal-200">
                    {l}
                    <button type="button" onClick={() => setLanguages(ls => ls.filter((_, idx) => idx !== i))} className="hover:text-red-500"><X className="w-3 h-3" /></button>
                  </span>
                ))}
              </div>
              <div className="flex gap-2">
                <input value={newLang} onChange={e => setNewLang(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter' && newLang.trim()) { e.preventDefault(); setLanguages(l => [...l, newLang.trim()]); setNewLang(''); } }}
                  placeholder={t('onboarding.addLanguage')}
                  className="flex-1 h-10 border border-gray-300 rounded-xl px-3 text-sm outline-none focus:border-teal-400" />
                <button type="button" onClick={() => { if (newLang.trim()) { setLanguages(l => [...l, newLang.trim()]); setNewLang(''); } }}
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center">
          ...
          <div className="relative z-10 w-full max-w-md mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden">
            ...
            <div className="px-8 py-6 text-center">
              ...
              <button onClick={() => { handleComplete(); setTimeout(() => navigate(`/doctor/${user?.id}`), 100); }}
                className="w-full mt-2 py-3 rounded-xl text-sm font-semibold text-teal-700 bg-teal-50 border border-teal-200 hover:bg-teal-100 transition-all flex items-center justify-center gap-2">
                <Eye className="w-4 h-4" /> {t('doctorProfile.viewPublicProfile')}
              </button>
            </div>
          </div>
        </div>
