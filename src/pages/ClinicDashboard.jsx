...
import {
  ...
  Stethoscope, ChevronRight, Loader2, Plus, Shield, ShieldAlert,
  AlertTriangle, X, UserPlus, ClipboardList, Star, Activity
} from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-0.5">
                {isVerified ? (
                  ...
                  <span className="inline-flex items-center gap-1 text-[10px] font-bold text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200/60">
                    <AlertTriangle className="w-3 h-3" /> {t('common.pendingVerification', 'Pending Verification')}
                  </span>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => {
                if (!isVerified) { setShowVerificationWarning(true); return; }
        ...
        {!isVerified && (
          <div className="mb-6 bg-amber-50 border border-amber-200/60 rounded-2xl p-4 flex items-start gap-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-semibold text-amber-800">{t('clinicDashboard.verifyTitle', 'Complete Verification')}</p>
              <p className="text-xs text-amber-700/80 mt-0.5">{t('clinicDashboard.verifyDesc', 'Submit your clinic documents to activate appointment creation and appear in search results.')}</p>
            </div>
            <Link to="/crm/settings" className="text-xs font-semibold text-amber-700 hover:text-amber-900 whitespace-nowrap underline underline-offset-2">
              {t('clinicDashboard.verifyNow', 'Verify Now')}
            </Link>
          </div>
        )}
            ...
            <div className="bg-white rounded-2xl shadow-2xl border border-gray-200/60 w-full max-w-md mx-4 overflow-hidden" onClick={e => e.stopPropagation()}>
              ...
              <div className="px-6 py-5">
                <p className="text-sm text-gray-600 leading-relaxed">
                  {t('clinicDashboard.verifyMessage', 'Your clinic must be verified before you can create appointments. Please submit your clinic documents for review.')}
                </p>
              ...
              <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/40 flex items-center justify-end gap-3">
                <button onClick={() => setShowVerificationWarning(false)}
                  className="px-4 py-2.5 text-sm font-medium text-gray-600 hover:bg-gray-100 rounded-xl transition-colors">
                  {t('common.close', 'Close')}
                </button>
                <button onClick={() => { setShowVerificationWarning(false); navigate('/crm/settings'); }}
