...
export default function MedStreamProfile() {
  ...
  if (state.error === 'notfound') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        ...
        <h1 className="text-lg font-bold text-gray-800">Bu hesap bulunamadı</h1>
        <p className="text-sm text-gray-500 mt-1">@{username} adlı bir profil mevcut değil.</p>
        <Link to="/medstream" className="mt-5 px-5 py-2.5 rounded-xl bg-teal-600 text-white text-sm font-semibold hover:bg-teal-700">MedStream'e dön</Link>
      </div>
    );
  }
  ...
  if (state.error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-6">
        <h1 className="text-lg font-bold text-gray-800">Profil yüklenemedi</h1>
        <p className="text-sm text-gray-500 mt-1">Lütfen daha sonra tekrar deneyin.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[680px] mx-auto bg-white min-h-screen border-x border-gray-100">
      ...
      <div className="flex justify-end px-4 pt-3 h-14">
        {isSelf ? (
          <Link to="/profile" className="px-4 py-1.5 rounded-full border border-gray-300 text-sm font-semibold hover:bg-gray-50">Profili düzenle</Link>
        ) : (
          <button
      ...
      <div className="px-4 pb-3">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-gray-500">
          ...
          {u.created_at && (
            <span className="inline-flex items-center gap-1"><CalendarDays className="w-4 h-4" />{joinedLabel(u.created_at)} tarihinde katıldı</span>
          )}
        </div>

        <div className="mt-3 flex items-center gap-5 text-sm">
          <span><span className="font-bold text-gray-900">{counts.following ?? 0}</span> <span className="text-gray-500">Takip</span></span>
          <span><span className="font-bold text-gray-900">{counts.followers ?? 0}</span> <span className="text-gray-500">Takipçi</span></span>
        </div>
      </div>
      ...
      <div className="flex border-b border-gray-100 sticky top-14 bg-white/95 backdrop-blur z-10">
        {TABS.map((t) => (
          <button
            ...
            className={`flex-1 py-3.5 text-sm font-semibold relative hover:bg-gray-50 ${tab === t.key ? 'text-gray-900' : 'text-gray-500'}`}
          >
            {t.label}
            {tab === t.key && <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-14 h-1 rounded-full bg-teal-600" />}
          </button>
        ))}
      </div>
        ...
        {postsLoading ? (
          ...
          <div className="py-16 text-center">
            <p className="text-sm font-semibold text-gray-600">
              {tab === 'media' ? 'Henüz medya yok' : 'Henüz gönderi yok'}
            </p>
            <p className="text-xs text-gray-400 mt-1">
              {canPost ? 'Bu hesap henüz paylaşım yapmadı.' : 'Bu hesap MedStream\'de paylaşım yapmıyor.'}
            </p>
          </div>
        ) : (
