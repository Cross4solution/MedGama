...
export default function SidebarPatient() {
  ...
  const pathname = stripLocale(rawPathname); // strip /en, /tr ... so active match works
  ...
  const NavItem = ({ to = undefined, href = undefined, icon: Icon, label, badge = undefined, external = false }) => {
    ...
    const accent = active ? <span className="absolute start-0 top-1/2 -translate-y-1/2 h-4 w-0.5 rounded-full bg-teal-500" /> : null;
    if (external && href) {
      return (
                ...
                <nav className="space-y-1">
                  {items.map((it) => {
                    ...
                    return (
                      ...
                      >
                        {active && <span className="absolute start-0 top-1/2 -translate-y-1/2 h-5 w-1 rounded-full bg-teal-600" />}
                        <span className="flex items-center gap-2.5 min-w-0 flex-1">
                          <span className={`relative flex items-center justify-center w-7 h-7 rounded-lg ${active ? 'bg-white shadow-sm' : 'bg-gray-100/80 group-hover:bg-gray-200/60'} transition-colors`}>
