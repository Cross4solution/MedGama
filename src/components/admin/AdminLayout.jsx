...
function AdminLoginGate() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const role = loggedUser?.role_id || loggedUser?.role || '';
      ...
      const msg = err?.message || err?.data?.message || 'Invalid credentials.';
      setError(msg);
    }
    setLoading(false);
  };
  ...
  const wrongRole = user && !ADMIN_ROLES.includes(user.role_id || user.role || '');

  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e1b4b 50%, #312e81 100%)' }}>
        ...
        <div className="bg-white/[0.07] backdrop-blur-xl rounded-2xl border border-white/10 p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-white/[0.06] border border-white/10 rounded-xl text-sm text-white placeholder:text-gray-500 focus:outline-none focus:ring-2 focus:ring-purple-500/40 focus:border-purple-500/50 transition-all pe-10"
                  placeholder="••••••••"
                  required
                />
                <button
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-500 hover:to-violet-500 text-white text-sm font-bold rounded-xl shadow-lg shadow-purple-500/25 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
          ...
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div ref={alertsRef} className="relative">
                ...
                {alertsOpen && (
                  <div className="absolute end-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-gray-200/60 z-50 overflow-hidden">
