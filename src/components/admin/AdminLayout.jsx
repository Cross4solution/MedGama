...
// Flatten for top-bar title lookup
...
const AdminLayout = ({ children }) => {
  ...
  const isActive = (path) => {
    ...
    return location.pathname.startsWith(path.split('?')[0]) && path !== '/admin';
  };
  ...
  const NavItem = ({ item }) => {
    ...
    if (hasChildren) {
      return (
        <div>
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${childActive ? 'bg-purple-500/20 shadow-[0_0_12px_rgba(168,85,247,0.3)]' : 'bg-white/5 group-hover:bg-white/10'}`}>
              <item.icon className={`w-4 h-4 ${childActive ? 'text-purple-400' : 'text-gray-500 group-hover:text-gray-300'}`} />
            </div>
    ...
    return (
      ...
      >
        <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${active ? 'bg-purple-500/20 shadow-[0_0_12px_rgba(168,85,247,0.3)]' : 'bg-white/5 group-hover:bg-white/10'}`}>
          <item.icon className={`w-4 h-4 ${active ? 'text-purple-400' : 'text-gray-500 group-hover:text-gray-300'}`} />
        </div>
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full" style={{ background: 'linear-gradient(180deg, #0f172a 0%, #1e1b4b 100%)' }}>
      {/* Logo */}
      <Link to="/admin" className="flex items-center gap-3 px-5 py-5 border-b border-white/5 hover:bg-white/5 transition-colors">
      ...
      <nav className="flex-1 overflow-y-auto px-3 pt-4 pb-2">
        {NAV_SECTIONS.map((section, si) => (
          <div key={section.title} className={si > 0 ? 'mt-5' : ''}>
            <p className="px-3 mb-2 text-[10px] font-bold text-gray-500/70 uppercase tracking-widest">{section.title}</p>
      ...
      <div className="px-3 mb-2">
        <Link
          to="/"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-gray-400 hover:text-white hover:bg-white/5 transition-all duration-200 group"
        >
      ...
      <div className="border-t border-white/5 p-4">
        <div className="flex items-center gap-3 px-2 py-2">
          <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-500 to-violet-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 shadow-lg shadow-purple-500/20">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="lg:pl-64 min-h-screen flex flex-col">
        {/* Top bar */}
        <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-xl border-b border-gray-200/60 px-4 lg:px-6 h-16 flex items-center">
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-[11px] bg-purple-50 text-purple-600 px-2.5 py-1 rounded-full font-semibold border border-purple-200">
                SuperAdmin
              </span>
            </div>
          </div>
